...
export function ConnectHubSpot() {
    ...
    const state: string | null = new URLSearchParams(window.location.search).get(
        "state"
    );
    ...
    const postData = async () => {
        ...
        try {
            setLoading(true);
            const response = await axios.post(`${BASE_URL}/Integration/hubspot/auth?userId=${uid}`, {
                code: code,
            });
            if (response.status === 200) {
                ...
                setMessage({
                    ...
                    message: "You have successfully connected with HubSpot. Click the button below to go to the listings and push your data to the CRM."
                });
            }
        } catch (error: any) {
            setCheckError(false)
            setMessage({
                ...
                message: "There was an error connecting to HubSpot. Please check your credentials and try again."
            });
        } finally {
        ...
        <div className="mb-20">
            <div className="d-flex justify-content-center p-5">
                <div className=''>
                    <img
                        alt='Logo'
                        src={toAbsoluteUrl('media/icons/HubSpot-Logo-2005.png')}
